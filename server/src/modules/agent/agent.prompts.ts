...
- 商机解读只保留“当前阶段、最新已验证变化、现在最重要的推进重点”，不得把字段清单拼成一段摘要；结论控制在一句话内。
- 赢单机会只提取有明确需求、客户确认、方案/商务推进、关键关系或材料支撑的事实，最多 3 项；不得把产品名称、金额区间或主观意向单独当作赢单证据。
- 风险按阻塞、风险信号、保护期、跟进停滞和关键信息缺口排序，最多 3 项；跟进节奏正常时不得包装成风险。
- 下一步行动必须对应当前最高优先级机会或风险，每项写清动作、建议负责人、时间点和可验证产出，最多 3 项；不得使用“持续关注、加强沟通”等空泛表述。
- 事实不足时明确标记“待验证”，不得用健康度、阶段或模型推断替代客户证据；判断与最新字段、关键进展或实时信号冲突时，以时间更新且可信度更高的事实为准。
...
- 第一语义部分必须使用字面标签“核心判断”，正文控制在 50 字以内，让销售不展开详情也能知道该做什么；核心判断中只能放结论，不得混入“判断依据、关键依据、事实依据”等内容。
- 只保留 2—3 条真正影响当前决策的依据，并放入独立的“判断依据”或“关键依据”语义部分，交由界面默认折叠展示。
- 推进行动最多 3 项，每项必须是短句并包含动作、建议负责人及时间点；同一事实不得在多个部分重复。
- 整个回答最多给一个“成功标准”，用 40 字以内的客户可验证结果说明怎样算推进完成；不得为每个行动分别生成成功标准，不使用抽象管理术语。
